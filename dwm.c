...
enum { SchemeNorm, SchemeSel, SchemeTagEmpty, SchemeTagOcc, SchemeTagSel,
       SchemeTitleNorm, SchemeTitleSel, SchemeLtSym }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagSel : occ & 1 << i ? SchemeTagOcc : SchemeTagEmpty]);
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("dwm.tagemptyfgcolor", tagemptyfgcolor);
        XRDB_LOAD_COLOR("dwm.tagemptybgcolor", tagemptybgcolor);

        XRDB_LOAD_COLOR("dwm.tagoccfgcolor", tagoccfgcolor);
        XRDB_LOAD_COLOR("dwm.tagoccbgcolor", tagoccbgcolor);

        XRDB_LOAD_COLOR("dwm.tagselfgcolor", tagselfgcolor);
        XRDB_LOAD_COLOR("dwm.tagselbgcolor", tagselbgcolor);
